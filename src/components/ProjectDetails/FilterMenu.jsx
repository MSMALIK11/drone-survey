...
const FilterMenu = ({ onItemClick, onResetFilter }) => {
  ...
  const { totalActiveFilterCount: selectedCount, filters } = useSelector(
        ...
        <div className="flex items-center justify-between px-4 w-[270px] h-[60px] border-b ">
          <div>
            <Checkbox
              size="small"
              name="is_admin"
              checked={filtersData?.is_admin}
              onClick={(e) => handleFilterAdminAndOwner(e)}
            />{" "}
            Admin
          </div>
          <div>
                ...
                <div className="flex">
                  <div>
                    <Checkbox
                      size="small"
                      name={`${item.key}.read`}
                      checked={filtersData[item.key]?.read}
                      onClick={handleReadWriteClick}
                    />{" "}
                    Read
                  </div>
                  <div>
                    <Checkbox
                      size="small"
                      name={`${item.key}.write`}
                      checked={filtersData[item.key]?.write}
                      onClick={handleReadWriteClick}
                    />{" "}
                    Write
                  </div>
