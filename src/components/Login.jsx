...
const Login = () => {
  ...
  const handleVerify = async (token) => {
    ...
    setToken(token);
    // try {
    //   // const res=awaits api.register.verifyTurnstile(secretkey,token)
  ...
  return (
    <Box className=" h-screen overflow-hidden flex items-center justify-center">
      <Box className="border-2 rounded-sm px-12 py-20 pb-12 w-[525px] min-h-[70%]">
        ...
        <p className="text-muted mt-4 text-sm">{t("label.accessMessage")}</p>

        <div className="mt-11">
          <div className="space-y-7">
            <InputControl
              error={error ? t("errorMessage.invalidEmailError") : null}
              name={"email"}
        ...
        <p className="text-muted text-md mt-2 ms-2">
          ...
          >
            <span className="text-softBlue !text-md">
              {" "}
              {t("label.signUpNow")}
            </span>
          </NavLink>
